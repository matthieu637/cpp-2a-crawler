 ...
 * @author David
 * 
 *         Premier objet hérité de Rudder (à compléter) Ce rudder parse tous les
 *         liens qu'il reçoit sans en changer l'ordre
 */
public class NaiveRudder extends Rudder {
	...
	public void addLink(List<String> links) {
		...
		for (String link : links) {
			PriorityLink pl = new PriorityLink(link, LinkQueue.getInstance().getPriorityOfLastElem() + 1);
			LinkQueue.getInstance().addPriorityLink(pl);
		}
	}
}
